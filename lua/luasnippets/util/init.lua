-- https://github.com/ray-x/go.nvim/blob/master/lua/go/snips.lua
...
function M.is_in_test_function()
  local ok, in_func = require('luasnippets.util.ts.go').in_func()
  if not ok then
    return false
  end

  local in_test_file = M.is_in_test_file()
  if not in_test_file then
    return false
  end

  return in_test_file and in_func
end
