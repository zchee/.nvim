-- token from https://github.com/neovim/nvim-lspconfig/blob/c8b90ae5cbe2/lsp/gopls.lua
...
function M.identify_go_dir(custom_args, on_complete)
...
---@return string?
function M.get_std_lib_dir()
  if std_lib and std_lib ~= '' then
    return std_lib
  end

  M.identify_go_dir({ envvar_id = 'GOROOT', custom_subdir = '/src' }, function(dir)
    if dir then
      std_lib = dir
    end
  end)
...
---@return string?
function M.get_mod_cache_dir()
  if mod_cache and mod_cache ~= '' then
    return mod_cache
  end

  M.identify_go_dir({ envvar_id = 'GOMODCACHE' }, function(dir)
    if dir then
      mod_cache = dir
    end
  end)
