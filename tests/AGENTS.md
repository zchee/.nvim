...
| `chrome_spec.lua` | `lua/config/chrome.lua` — the hand-rolled statusline and tabline that replaced lualine+bufferline: module-load budget, the component surface, buffer-id numbers, the modified/readonly markers, diagnostic strings and their insert-mode deferral, insert-after-current tabline ordering, bufferline's name-truncation rules, the `%@` click handler's button discrimination, the blanked `snacks_picker_input` statusline and `%` escaping in filenames. Parity contract in `.omc/plans/round3-chrome-parity.md` |
...
| `perf/hl_dump_spec.lua` | Colorscheme-parity falsifier (round-2 plan R3.1, acceptance criterion 4) — runs `script/hl-dump.lua` in a fresh full-config headless nvim and byte-compares the canonical dump against the frozen pre-port fixture `perf/fixtures/hl_baseline.txt` (VimL colorscheme + the 97 `highlight.lua` overrides), twice: first paint and after a `:colorscheme` re-apply (`--reapply`), reporting the first divergent line. A nightly bump that changes default-derived groups means regenerating the fixture with `nvim --headless -l script/hl-dump.lua <fixture>` |
| `perf/metrics_probe_spec.lua` | uv-metrics stall probe falsifier (round-3.5 plan item 1) — pins the mechanism behind the stall probe embedded in `script/perf-report.sh` (a bash heredoc, so not `require()`-able) with identical inline wiring: `loop_configure("metrics_idle_time")` returns 0, `metrics_idle_time()` (nanoseconds) grows across an idle 200 ms `vim.wait`, `metrics_info()` exposes `loop_count`, and the prepare/check handle pair (check = poll returned, prepare = about to block, so check→next-prepare is one loop turn's active stretch) registers a synthetic 30 ms hrtime busy-loop as a ≥25 ms stall |
...
| `perf/trace_export_spec.lua` | `script/perf-trace.lua` — the round-3.5 Perfetto/Chrome trace-event exporter: runs it end-to-end (the exporter spawns its own full-config child, fires UIEnter manually to arm the warmup, and merges the `--startuptime` log with lazy.nvim per-plugin load times and warmup `tick_ms`), then pins the output contract — the file round-trips through `vim.json.decode`, top level is `{traceEvents, displayTimeUnit="ms"}`, every event carries name/ph/ts/pid/tid with `dur` on `ph="X"`, ts is sane µs (0 ≤ ts < 10⁹) and non-decreasing per tid, and each source (`startuptime`/`lazy`/`warmup`, via `args.source`) contributed ≥1 event. Durations are reported, never asserted |
| `luasnippets_parse_spec.lua` | Every file under `lua/luasnippets/` loads without raising — the set of a filetype is only executed on that filetype's first InsertEnter, so a broken file stays invisible until someone opens that filetype and finds all its snippets missing (the case that motivated it: `fmta` reads `<>` as delimiters, so a literal `>` must be doubled). Both file shapes pass: returning a snippet table or registering imperatively with `ls.add_snippets`. Appends lazy's `LuaSnip` dir to the rtp and skips cleanly when the plugin is not installed |
| `perf/ui_latency_spec.lua` | Embed UI latency client smoke (round-3.5 plan item 3) — runs `script/ui-latency.lua --clean --socket-free` end to end via `vim.system` (60 s bound): the msgpack-RPC UI attaches to a spawned `nvim --embed -u NONE -i NONE`, `attach_to_first_flush_ms` and `input_to_flush_ms_median` parse from stdout inside the generous (0, 5000) ms smoke bounds, and the script exits 0 (deadline plumbing never hangs). Timing numbers are reported by `script/perf-report.sh`, never asserted here |
| `perf/warmup_spec.lua` | `lua/config/warmup.lua` — the round-2 cooperative insert-stack warmup, driven entirely through injected recorder deps (fake lazy.load / is_loaded / scheduler / tagger, so no real plugin loads): unit shape (7-plugin order, blink.cmp terminal), one-plugin-load-per-scheduled-tick discipline, the InsertEnter abort flag before and between ticks, already-loaded skip + terminal short-circuit idempotency, gate deferral/timeout, non-fatal prewarm failures, fatal plugin-load failures, the `vim.g.warmup_loaded` tag shape, and UIEnter arming via `exec_autocmds` |
