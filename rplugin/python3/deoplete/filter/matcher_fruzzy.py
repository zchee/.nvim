...
class Filter(Base):
    ...
    def filter(self, context):
        if not context["candidates"] or not context["input"] or self.fruzzy is False:
        ...
        results = self.get_fruzzy_result(
            qry,
            candidates,
            1000,
            key=key,
    ...
    def _init_fruzzy(self, context):
        ...
        if found:
            ...
            errmsg = (
                ...
                "You must install/build fruzzy with Python 3 support." % (fname)
            )
        if errmsg:
            self.fruzzy = False
        return errmsg

    def get_fruzzy_result(
        self, q, c, limit, key=None, ispath=True, buf=0, sortonempty=True
    ):
        relname = ""
