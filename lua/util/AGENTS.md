...
`fast_switch`, `contains`, `dump`). `lua/util/watch.lua` is a standalone
`fswatch`-backed file watcher implementing the `vim._watch` callback
contract, not re-exported through `util/init.lua`.
...
| `watch.lua` | `fswatch(path, opts, callback)` — CLI-backed alternative to libuv file watching |

## For AI Agents

### Working In This Directory
- `require("util")` returns the `init.lua` module table; `watch.lua` is
  required separately as `require("util.watch")` (currently unreferenced
  elsewhere in the repo — verify before assuming it is wired up).
...
### External
`fswatch` CLI binary (Homebrew) for `watch.lua`; `vim.uv`/`vim.fs` (Neovim
